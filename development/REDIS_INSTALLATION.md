...
3. **Install Python Client (venv)**
   ...
   **Important**: Make sure the venv is activated (you should see `(.venv)` in your prompt) before running pip install.

## Verification

### Step 1: Check Redis Server is Running
```bash
# Test connection (doesn't require venv)
redis-cli ping
# Should return: PONG
```

**If `redis-cli` not found**: Redis server may not be installed. Install with `brew install redis` first.

### Step 2: Check Python Client in venv
```bash
cd /Users/scottroberts/Library/CloudStorage/Dropbox/app_development/Construction_Suite/apps/digidoc
...
**If you get `ModuleNotFoundError: No module named 'redis'`**:
- Make sure venv is activated (check for `(.venv)` in prompt)
- Install requirements: `pip install -r requirements_ocr.txt`
- Or install redis directly: `pip install redis>=5.0.0 rq>=1.0.0`
